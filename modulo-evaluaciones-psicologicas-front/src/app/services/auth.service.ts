...
export class AuthService {
  ...
  constructor() {
    ...
    if (token) {
      ...
      if (username || decoded.roles.length) {
        this.userSig.set({ username, active: true, roles: decoded.roles });
      }
    }
  }
    ...
    return this.http.get<UserDTO>(`${this.authUrl}/current-user`).pipe(
      ...
      map(u => {
        ...
        const flattenedRoles = [...rawRoles, ...decodedRoles]
          ...
          .filter(Boolean);
        const normalizedRoles = this.normalizeRoles(flattenedRoles);
  ...
  private decodeToken(token: string): DecodedTokenResult {
    try {
      ...
      return { username: '', roles: [], expiresAt: null };
    }
  }
  ...
  private buildUserFromDecoded(decoded: DecodedTokenResult, fallbackUsername: string): UserDTO {
    const normalizedRoles = this.normalizeRoles(decoded.roles);
    const username = decoded.username || fallbackUsername;
    return { username, roles: normalizedRoles, active: true };
  }
