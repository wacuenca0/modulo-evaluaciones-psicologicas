...
export class CatalogosAdminComponent implements OnInit {
  ...
  readonly nivelCalculado = computed(() => {
    ...
    if (parent && parent.nivel !== null && parent.nivel !== undefined) {
  ...
  private buildChildrenMap(listado: CatalogoCIE10DTO[]): Map<string, CatalogoCIE10DTO[]> {
    ...
    mapa.forEach(children => children.sort((a, b) => a.codigo.localeCompare(b.codigo)));
    return mapa;
  }
  ...
  private toCreatePayload(base: { codigo: string; nombre: string; descripcion: string; categoriaPadre: string | null; nivel: number; activo: boolean }): CreateCatalogoCIE10Payload {
    return {
      codigo: base.codigo,
      nombre: base.nombre,
      descripcion: base.descripcion,
      categoriaPadre: base.categoriaPadre,
      nivel: base.nivel,
      activo: base.activo
    };
  }
